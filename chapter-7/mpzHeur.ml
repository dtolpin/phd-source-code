(* available heuritics for inverse-weighted 15 puzzle *)

open MpzHeurManhattan
open MpzHeurConflict
...
module LA = Lookahead.Make(MpzProblem)
...
let rec make_heur' name =
  ...
  | name when String.contains name '^' ->
    ...
    and depth = float_of_string (String.sub name (k+1) (String.length name - k - 1))
