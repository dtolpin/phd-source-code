...
let rec error_message input pos messages width indent = 
  ...
  let buf = Buffer.create 160 in
    ...
    if unexp != [] then 
      Buffer.add_string buf
        (ind ^ "Unexpected " ^ (String.concat_conj "," "and" unexp) ^ "\n");
    if exp != [] then 
      Buffer.add_string buf
        (ind ^ "Expecting " ^ (String.concat_conj "," "or" exp) ^ "\n");
    if msg != [] then 
...
let many_rev p = 
  many_fold_apply (fun xs x -> x :: xs) [] identity p

let many1_rev p = 
  many1_fold_apply (fun xs x -> x :: xs) [] identity p
...
let many_fold_left f a =
  many_fold_apply f a identity

let many1_fold_left f a =
  many1_fold_apply f a identity
...
let none_of str = 
  satisfy (non (String.contains str))
...
let expression operators term = 
  ...
  let make_parser term ops =
    ...
    let prefix_p  = opt identity prefix_op in
    let postfix_p = opt identity postfix_op in
