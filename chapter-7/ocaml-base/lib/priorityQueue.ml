...
let remove_first h = ignore (extract_first h)

let remove h x = 
  try ignore (remove_index h (Hashtbl.find h.indices x))
  with Not_found -> ()
...
struct
  ...
  let remove h x = 
    try ignore (remove_index h (Tbl.find h.indices x))
    with Not_found -> ()
