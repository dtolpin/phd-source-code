...
val parse_channel: ('a, 's) parser -> in_channel -> 's -> 'a result
...
val return: 'a -> ('a, 's) parser
(** [return x] always succeeds with the result [x] without consuming any
    input.  *)

val try_return: ('a -> 'b) -> 'a -> string -> 's state -> ('b, 's) parser
...
val try_return2: ('a -> 'b -> 'c) -> 'a -> 'b -> string -> 's state -> ('c, 's) parser
(** A variant of [try_return] for functions with two parameters. *)

val try_return3: ('a -> 'b -> 'c -> 'd) -> 'a -> 'b -> 'c -> string -> 's state -> ('d, 's) parser
(** A variant of [try_return] for functions with three parameters. *)

val fail: string -> ('a, 's) parser
...
val (<<): ('a, 's) parser -> ('b, 's) parser -> ('a, 's) parser
(** [p << q] is equivalent to [p >>= (fun x -> q >> return x)]. *)

val (>>$): ('a, 's) parser -> 'b -> ('b, 's) parser
...
val (|>>): ('a, 's) parser -> ('a -> 'b) -> ('b, 's) parser
(** [p |>> f] is equivalent to [p >>= (fun x -> return (f x))]. *)

val pipe2: ('a, 's) parser -> ('b, 's) parser -> ('a -> 'b -> 'c) -> ('c, 's) parser
(** A variant of [(|>>)] for functions with two parameters. *)

val pipe3: ('a, 's) parser -> ('b, 's) parser -> ('c, 's) parser -> ('a -> 'b -> 'c -> 'd) -> ('d, 's) parser
(** A variant of [(|>>)] for functions with three parameters. *)

val pipe4: ('a, 's) parser -> ('b, 's) parser -> ('c, 's) parser -> ('d, 's) parser -> ('a -> 'b -> 'c -> 'd -> 'e) -> ('e, 's) parser
(** A variant of [(|>>)] for functions with four parameters. *)

val (<|>): ('a, 's) parser -> ('a, 's) parser -> ('a, 's) parser
(** [p <|> q] first applies [p].  If [p] fails without consuming input, it
    applies [q]. *)

val choice: ('a, 's) parser list -> ('a, 's) parser
(** [choice \[p1; p2; ...; pn \]] is equivalent to [p1 <|> p2 <|> ... <|> pn
    <|> zero]. *)

val attempt: ('a, 's) parser -> ('a, 's) parser
...
val (<?>): ('a, 's) parser -> string -> ('a, 's) parser
...
val not_followed_by: ('a, 's) parser -> string -> (unit, 's) parser
...
val opt: 'a -> ('a, 's) parser -> ('a, 's) parser
(** [opt x p] is equivalent to [p <|>$ x]. *)

val option: ('a, 's) parser -> ('a option, 's) parser
(** [option p] is equivalent to [p >>= (fun r -> return (Some r)) <|>$
    None]. *)

val optional: ('a, 's) parser -> (unit, 's) parser
(** [optional p] is equivalent to [p >>$ () <|>$ ()]. *)

val try_skip: ('a, 's) parser -> (bool, 's) parser
(** [try_skip p] is equivalent to [p >>$ true <|>$ false]. *)

val pair: ('a, 's) parser -> ('b, 's) parser -> ('a * 'b, 's) parser
(** [pair p q] is equivalent to [ p >>= (fun x -> q >>= (fun y -> return (x,
    y)))]. *)

val many: ('a, 's) parser -> ('a list, 's) parser
...
val chain_right: ('a, 's) parser -> ('a -> 'a -> 'a, 's) parser -> 'a -> ('a, 's) parser
...
val chain_right1: ('a, 's) parser -> ('a -> 'a -> 'a, 's) parser -> ('a, 's) parser
...
val sep_end_by1: ('a, 's) parser -> ('b, 's) parser -> ('a list, 's) parser
(** [sep_end_by1 p sep] parses one or more occurrences of [p], separated and
    optionally ended by [sep].  It returns a list of the results returned by
    [p]. *)

val end_by: ('a, 's) parser -> ('b, 's) parser -> ('a list, 's) parser
...
val end_by1: ('a, 's) parser -> ('b, 's) parser -> ('a list, 's) parser
(** [end_by1 p sep] parses one or more occurrences of [p], separated and ended
    by [sep].  It returns a list of the results returned by [p]. *)

val many_until: ('a, 's) parser -> ('b, 's) parser -> ('a list, 's) parser
...
val skip_many_until: ('a, 's) parser -> ('b, 's) parser -> (unit, 's) parser
...
val get_pos: (pos, 's) parser
(** Returns the current position. *)

val register_nl: int -> int -> (unit, 's) parser
...
val get_user_state: ('s, 's) parser
(** Returns the current user state of the parser. *)

val set_user_state: 's -> (unit, 's) parser
(** Sets the current user state of the parser. *)

val update_user_state: ('s -> 's) -> (unit, 's) parser
...
val char: char -> (char, 's) parser
(** [char c] parses the character [c]. *)

val skip_char: char -> (unit, 's) parser
(** [char c] parses the character [c] and returns [()]. *)

val any_char: (char, 's) parser
...
val string: string -> (string, 's) parser
(** [string s] parses the string [s] and returns it. *)

val skip_string: string -> (unit, 's) parser
(** [string s] parses the string [s] and returns [()]. *)

val any_string: int -> (string, 's) parser
...
val many1_chars: (char, 's) parser -> (string, 's) parser
(** [many1_chars p] parses one or more occurrences of [p] and returns a string
    of the results returned by [p].

    @raise Failure if [p] accepts the empty string. *)

val skip_many_chars: (char, 's) parser -> (unit, 's) parser
(** [skip_many_chars p] is equivalent to [many_chars p |>> ignore]. *)

val skip_many1_chars: (char, 's) parser -> (unit, 's) parser
(** [skip_many1_chars p] is equivalent to [many_chars p |>> ignore]. *)

val many_chars_until: (char, 's) parser -> (char, 's) parser -> (string, 's) parser
...
val skip_many_chars_until:
  (char, 's) parser -> (char, 's) parser -> (unit, 's) parser
(** [skip_many_until p q] parses zero or more occurrences of [p] until [q]
    succeeds and returns [()].  It is equivalent to [skip_many
    (not_followed_by q "" >> p) << q].  Note that [q] is parsed twice and
    should therefore not have side effects. *)

val satisfy: (char -> bool) -> (char, 's) parser
...
val satisfy_l: (char -> bool) -> string -> (char, 's) parser
(** [satisfy_l p label] is equivalent to [satisfy p <?> label]. *)

val skip_satisfy: (char -> bool) -> (unit, 's) parser
(** [skip_satisfy p] is equivalent to [satisfy |>> ignore]. *)

val skip_satisfy_l: (char -> bool) -> string -> (unit, 's) parser
(** [skip_satisfy_l p label] is equivalent to [satisfy_l p label |>>
    ignore]. *)

val nsatisfy: int -> (char -> bool) -> (string, 's) parser
...
val many_satisfy: (char -> bool) -> (string, 's) parser
(** [many_satisfy p] is equivalent to [many (satisfy p)]. *)

val many1_satisfy: (char -> bool) -> (string, 's) parser
(** [many1_satisfy p] is equivalent to [many1 (satisfy p)]. *)

val skip_many_satisfy: (char -> bool) -> (unit, 's) parser
(** [skip_many_satisfy p] is equivalent to [skip_many (satisfy p)]. *)

val skip_many1_satisfy: (char -> bool) -> (unit, 's) parser
(** [skip_many1_satisfy p] is equivalent to [skip_many1 (satisfy p)]. *)

val next_char_satisfies: (char -> bool) -> (unit, 's) parser
(** [next_char_satisfies p] succeeds without consuming input if [p] returns
    ...
    with an [Unkown_error]. *)

val prev_char_satisfies: (char -> bool) -> (unit, 's) parser
...
val spaces: (unit, 's) parser
(** [spaces] is equivalent to [skip_many_chars space]. *)

val spaces1: (unit, 's) parser
...
type ('a, 's) operator =
    Infix of (('a -> 'a -> 'a, 's) parser * assoc)   (** Infix operator. *)
  | Prefix of ('a -> 'a, 's) parser                  (** Prefix operator. *)
  | Postfix of ('a -> 'a, 's) parser                 (** Postfix operator. *)
...
val expression: (('a, 's) operator list) list -> ('a, 's) parser -> ('a, 's) parser
